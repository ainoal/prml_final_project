...
function [data_out] = rotate_data(data,min_thet,max_thet)
    %input: mxn matrix, minimum angle in rad (double), maximum angle in rad
    %(double). Randomly rotates data around z axis within defined angle
    %range

    %ignoring velocity data as it can not be rotated, as we only want to
    %roation with position features
